...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (fork() == 0) { // if you're a child
            ...
            close(listener_d); // don't need to listen, only using connection socket

            if (say(connect_d, intro_msg) == -1) {
                close(connect_d);
                puts("Failed to communicate with client. Socket connection closed.\n");
                continue;
            }
            ...
            close(connect_d); // child closes socket to its client
            puts("A client has disconnected.");
            exit(0); // end child process
        }
        // close the original copy of the socket, since no child did
        // lazy kids
        close(connect_d);
    }
    return 0;
