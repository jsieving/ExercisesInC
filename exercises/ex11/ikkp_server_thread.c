...
{
    // puts("A client has been swiftly and silently murdered!");
    // raise(SIGSEGV); // Where's a good segfault when you need one?
...
/* Create a new thread */
pthread_t make_thread(void *(*entry)(void *), int *socket_ptr) {
    int ret;
    pthread_t thread;

    ret = pthread_create(&thread, NULL, entry, (void *) socket_ptr);
    if (ret != 0) {
...
/* Join a new thread with the parent thread */
...
/* communicate with the client, then exit */
void* child_code(void *socket_ptr) {
  int connect_d = *((int*) socket_ptr);
  int* res = malloc(sizeof(int));
  *res = 0;
  char buf[255];

  *res = say(connect_d, "Internet Knock-Knock Protocol Server\n\
Knock, knock.\n");
  if (*res == -1) {
      close(connect_d);
      pthread_exit(&res);
  ...
  *res = say(connect_d, "Surrealist giraffe.\n");
  if (*res == -1) {
      close(connect_d);
      pthread_exit(&res);
  }
  ...
  *res = say(connect_d, "Bathtub full of brightly-colored machine tools.\n");
  if (*res == -1) {
      close(connect_d);
      pthread_exit(res);
  }
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        int* connect_d_ptr = malloc(sizeof(int));
        *connect_d_ptr = open_client_socket();
        puts("A client has connected.");
        make_thread(child_code, connect_d_ptr);
        children++;
    }
    return 0;
}
